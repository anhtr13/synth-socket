...
func HandleAcceptFriendRequest(w http.ResponseWriter, r *http.Request) {
	...
	err = conf.RD_Client.SAdd(
		r.Context(),
		fmt.Sprintf("%s:%s:%s", conf.KEY_USER, friendship.User1ID.String(), conf.USER_FRIENDS),
		friendship.User2ID.String(),
	).Err()
	err = conf.RD_Client.SAdd(
		r.Context(),
		fmt.Sprintf("%s:%s:%s", conf.KEY_USER, friendship.User2ID.String(), conf.USER_FRIENDS),
